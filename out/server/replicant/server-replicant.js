    ...
    constructor(name, namespace, opts = {}, startingValue = undefined) {
        ...
        function getBundlePath() {
            if (project_type_1.isLegacyProject) {
                return path.join((0, nodecg_root_1.getNodecgRoot)(), "bundles", namespace);
            }
            const rootPackageJson = fs.readFileSync(path.join(root_path_1.rootPath, "package.json"), "utf-8");
            if (JSON.parse(rootPackageJson).name === namespace) {
                return root_path_1.rootPath;
            }
            else {
                const bundlesDir = path.join(root_path_1.rootPath, "bundles");
                const bundlesDirStat = fs.existsSync(bundlesDir)
                    ? fs.statSync(bundlesDir)
                    : null;
                if (bundlesDirStat === null || bundlesDirStat === void 0 ? void 0 : bundlesDirStat.isDirectory()) {
                    const bundles = fs.readdirSync(path.join(root_path_1.rootPath, "bundles"), {
                        withFileTypes: true,
                    });
                    for (const bundleDir of bundles) {
                        ...
                        const bundlePath = path.join(root_path_1.rootPath, "bundles", bundleDir.name);
        ...
        if (schemaPath) {
            if (path.isAbsolute(schemaPath)) {
                ...
                absoluteSchemaPath = path.join(project_type_1.isLegacyProject ? (0, nodecg_root_1.getNodecgRoot)() : root_path_1.rootPath, schemaPath);
            }
        }
        else {
